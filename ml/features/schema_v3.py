...
from app.feature_contract import feature_values as feature_values_v2
...
def compute_features(row: dict[str, Any]) -> dict[str, float | str]:
    ...
    categories = {name: base.pop(name) for name in CATEGORICAL_FEATURES}
    derived = semantic_derived_values(
        contradiction=float(row.get("semantic_contradiction", 0)),
        neutral=float(row.get("semantic_neutral", 0)),
        entailment=float(row.get("semantic_entailment", 1)),
    )
    values = {**base, **derived, **categories}
